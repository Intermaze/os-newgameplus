...
int8_t read(struct EXT2DriverRequest request)
{
  ...
  while (offset < BLOCK_SIZE && entry->inode != 0)
  {
    ...
    if (is_directory_entry_same(entry, request, true))
    {
      ...
      read_blocks(&inode_table_buf, bgd_table.table[bgd].inode_table, INODES_TABLE_BLOCK_COUNT);
      node = &inode_table_buf.table[local_idx];
      if (request.buffer_size < node->size)
      {
        // not enough buffer
        return 2;
      }

      load_inode_blocks(request.buf, node->block, node->size);
...
int8_t write(struct EXT2DriverRequest *request)
{
  ...
  else
  {
    ...
    new_node.size = request->buffer_size;
    new_node.dir_acl = 0;
...
int8_t delete(struct EXT2DriverRequest request)
{
  ...
  while (!found)
  {
    ...
    if (is_directory_entry_same(entry, request, !request.is_directory))
    {
      found = true;
    }
    else
  ...
  if (request.is_directory && !is_directory_empty(entry->inode))
    return 2;
